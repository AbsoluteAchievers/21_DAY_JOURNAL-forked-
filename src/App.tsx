...
function App() {
  ...
  const [salesActivityCount, setSalesActivityCount] = useState<string>('');
  ...
  const daysCompleted = useMemo(() => {
    ...
    entries.filter(e => e.date && e.focusScore !== undefined && e.focusScore !== null).forEach(entry => uniqueDates.add(entry.date.split('T')[0]));
        ...
        const unsubscribe = onAuthStateChanged(authentication, async (user) => {
          if (user) {
            ...
            try {
              ...
              setMessage('Error signing in. Please check console for details.');
            }
          }
          setIsLoading(false);
        });
    ...
    if (db && userId) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        const fetchedEntries: JournalEntry[] = snapshot.docs.map(doc => ({
          id: doc.id,
          date: doc.data().date,
          focusScore: doc.data().focusScore,
          topPriority: doc.data().topPriority,
          salesActivityCount: doc.data().salesActivityCount,
          coreHabitDone: doc.data().coreHabitDone,
          winOfTheDay: doc.data().winOfTheDay,
          lessonLearned: doc.data().lessonLearned,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parseInt(focusScore)) || parseInt(focusScore) < 1 || parseInt(focusScore) > 5) {
      setMessage('Focus Score must be a number between 1 and 5.');
      return;
    }
    if (isNaN(parseInt(salesActivityCount)) || parseInt(salesActivityCount) < 0) {
